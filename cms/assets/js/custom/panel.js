...
const deployFailedNoAccessRights =
  "Keine Berechtigung. Vergewissern Sie sich, dass Sie mit einem Admin Konto eingeloggt sind.";
  ...
  window.innerWidth > 500 ? "70%" : "100%"
    ...
    <article>
        ...
        <div style="display: grid; place-items: center">
            <button class="deploy-button dark" id="confirmation-deployment">${deployConfirmTextBtn}</button>
        </div>
        <div style="display: grid; place-items: center">
 ...
 * Listeners
 */
overlayConfirmationBtn.addEventListener("click", (e) => {
      ...
      if (json.retval === 0 || json.retval === "0") {
        ...
        overlayCollapsibleLogContent.innerHTML =
          Array.isArray(json.message) && json.message.length > 0
            ? json.message.reduce(
                (prevValue, currentValue) => prevValue + "<br/>" + currentValue
              )
            : json.message;
    ...
    .catch((e) => {
      ...
      if (!!e.cause) {
        if (typeof e.cause === "object") {
          overlayFailure.innerHTML = getDeployFailedMsgWithCode(e.cause.code);
